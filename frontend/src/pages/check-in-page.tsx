...
export function CheckInPage() {
    ...
    const [message, setMessage] = useState('Realizando check-in...')
    ...
    useEffect(() => {
        ...
        checkInAttendee(token)
            ...
            .catch((error: Error) => {
                setStatus('error')
                setMessage(getFriendlyErrorMessage(error, 'Nao foi possivel realizar o check-in.'))
            })
    }, [token])
            ...
            {status === 'success' && (
                <section className="border border-white/10 rounded-lg p-5 flex flex-col gap-4 bg-white/[0.02]">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-2xl font-bold">Check-in realizado</h1>
                    </div>
